...
RUN powershell.exe -Command "if (!(Test-Path -Path $env:TEMP_DIR)) { New-Item -ItemType Directory -Path $env:TEMP_DIR; }"
...
RUN powershell -Command "if (!(Test-Path -Path C:\build)) { New-Item -ItemType Directory -Path C:\build; } ; & \"$env:CMAKE_PATH\" -G \"Visual Studio $env:VS_VERSION Win64\" -DCMAKE_BUILD_TYPE=Release -S C:\app -B C:\build"

# Build the project
WORKDIR C:/build
RUN powershell -Command "$env:CMAKE_PATH --build C:\build --config Release; if ($?) { Write-Host 'Build succeeded' } else { Write-Error 'Build failed' }"

# Set the default command to execute the built application
CMD ["powershell", "-File", "C:\\app\\scripts\\windows\\run.ps1"]